...
<a href="#section-anspruchsvolle-Aufgaben"> <i class="fa-solid fa-arrow-down"></i>  Anspruchsvolle Aufgaben</a><br>
...
```{r ex-debugging-4, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 24}
data <- data.frame(
...
# Der Code ab hier ist korrekt!
# Hier wird die Consolenausgabe abgefangen mit capture.output,
# sodass der Output auch gespeichert werden kann in einer Variable.
results <- capture.output(
  bank_status(data, "Lizzy"),
  bank_status(data, "Beatrice"),
  bank_status(data, "Peter"))
results
```
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Entferne alle Zeilen mit fehlenden Werten aus dem Datensatz und prüfe, ob sich keine fehlenden Werte mehr im Datensatz befinden.*
...
data_health <- na.omit(data_health)
colSums(is.na(data_health))
```

### Anspruchsvolle Aufgaben <a href="#section-übergeordnete-übungen"> <i class="fa-solid fa-arrow-up"></i></a>

...
Das [*tidyverse*](https://www.tidyverse.org/) ist eine Sammlung von open-source R-Paketen, die speziell für die Datenaufbereitung und -visualisierung entwickelt wurden.
...
*Benutzerfreundlichkeit und Lesbarkeit:* Das tidyverse bietet eine intuitive Syntax, die mit Pipes (%>%) arbeitet. Dies ermöglicht eine klare und lesbare Darstellung von Datenmanipulationsprozessen, wodurch auch komplexe Analysen nachvollziehbar bleiben.
...
Er ermöglicht es, mehrere Funktionen nacheinander auf einen Datensatz anzuwenden, ohne dabei das Datensatz-Objekt in jedem Funktionsaufruf explizit wiederholen zu müssen.
...
Der Pipe-Operator (`%>%`) ist ein mächtiges Werkzeug, das den Code übersichtlicher macht und mehr der Denkweise des Menschen entspricht.
 ...
 *Schreibe den Code um und verwende hier den Pipe Operator `%>%`. Ändere ansonsten so wenig, wie möglich*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Entferne alle Zuweisungen `<-`, welche durch die Pipe ersetzt werden.</div>
 ...
 *Wähle die Spalten "income", "id" und "school_years" aus*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze `select()`.</div>
  <div class="tip-content"> Wähle die Spaltennamen aus, Anführungszeichen sind dabei nicht notwendig.</div>
...
<div class="tip">
  ...
  <div class="tip-content"> Nutze `select()`.</div>
  <div class="tip-content"> Nutze `ends_with()`.</div>
 ...
 *Wähle alle Spalten aus, welche eine Ziffer enthalten*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze `matches()`.</div>
  <div class="tip-content"> Verwende eine Regex-Syntax für `matches()`.</div>
  <div class="tip-content"> Nutze `".*[0-9]+.*"` als Regex-Syntax.</div>
 ...
 *Wähle alle Studenten aus*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze `filter()`.</div>
 ...
 *Wähle Personen aus, die älter als 40 Jahre sind oder ein Einkommen über 4000€ haben*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze das Symbol `|` für eine oder-Logik.</div>
...
Die Funktion `arrange()` verändert die Zeilen eines Datensatzes basierend auf einer oder mehreren Spalten.
 ...
 *Sortiere die Daten nach dem Einkommen. Das höchste Einkommen soll in der ersten Zeile erscheinen.*
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze `desc()`.</div>
 ...
 *Benenne die Spalte "intelligence" in "iq_score" um*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Die Zuweisungsreihenfolge ist: `alter_name = neuer_name`.</div>
 ...
 *Erstelle eine neue Spalte namens "name" und setze "unbekannt" als Wert*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze `mutate()`.</div>
  <div class="tip-content"> Nutze `.after = id`.</div>
 ...
 *Überschreibe die Werte von "income", sodass R den Wert als fehlend ("not available") begreift*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze `mutate()`.</div>
  <div class="tip-content"> Nutze die Zuweisungen zu NA.</div>
 ...
 *Erstelle eine neue Spalte namens "mean_income" und berechne dort den Durchschnitt des Einkommens aller Personen*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze `mutate()`.</div>
  <div class="tip-content"> Nutze `.after = id`.</div>
  <div class="tip-content"> Nutze `mean()`.</div>
 ...
 *Aggregiere die Tabelle: Erstelle dabei eine neue Spalte namens "mean_income" und berechne dort den Durchschnitt des Einkommens aller Personen*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze `summarize()`.</div>
  <div class="tip-content"> Nutze `mean()`.</div>
 ...
 *Berechne in Abhängigkeit des Geschlechts das durchschnittliche Einkommen (aggregiere die Daten). Benenne die neue Spalte "mean_income"*.
 ...
 <div class="tip">
  ...
  <div class="tip-content"> Nutze die Funktion `group_by()`.</div>
  <div class="tip-content"> Nutze die Funktion `summarize()`.</div>
  <div class="tip-content"> Nutze die Funktion `mean()`.</div>
  <div class="tip-content"> Nutze diesen Aufbau: 
  <pre>df %>% 
  group_by() %>% 
  summarize()</pre></div>
 ...
 *Berechne in Abhängigkeit des Geschlechts das durchschnittliche Einkommen (aggregiere die Daten). Benenne die neue Spalte "mean_income". Berechne danach noch das durchschnittliche Alter pro Geschlecht (speichere das Ergebnis in "mean_age") ab. Löse danach die Gruppenstruktur auf*.
  ...
  <div class="tip-content"> Nutze die Funktion `summarize()`.</div>
  <div class="tip-content"> Nutze die Funktion `mean()`.</div>
  <div class="tip-content"> Nutze die Funktion `ungroup()`.</div>
  <div class="tip-content"> Nutze diesen Aufbau: 
  <pre>df %>% 
  group_by() %>% 
  summarize() %>% 
...
```
```{r, ref.label=I('setup-join-df'), eval=TRUE, echo=FALSE, exercise.eval=FALSE, exercise.lines = 16}
```
...
left_join(students, enrollments, by = "student_id", suffix = c("_stud", "_enroll"))
```
...
anti_join(enrollments, students, by = "student_id")
```
...
df_id <- data.frame(
  proband_id = 1:8,
  first_name = c("Anna", "Ben", "Chris", "Dana", "Eva", "Felix", "Gina", "Hans"),
  last_name = c("Müller", "Schmidt", "Weber", "Fischer", "Meier", "Becker", "Hoffmann", "Schneider"),
...
```
```{r, ref.label=I('setup-join-ex'), eval=TRUE, echo=FALSE, exercise.eval=FALSE}
```
 ...
 *Fülle den Datensatz "df_id" auf mit allen Infos aus "df_med"*.
  ...
  <div class="tip-content"> Nutze eine join Funktion `*_join(df_id, df_med,____)`.</div>
  <div class="tip-content"> Nutze `left_join(____)`.</div>
  <div class="tip-content"> Nutze `by = "proband_id"`.</div>
...
Der Datensatz ist nun länger geworden (hat mehr Zeilen) und schmaler (hat weniger Spalten).
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
 *Verwandle den Datensatz "fruits_wide" ins Long-Format, sodass du eine Spalte `Monat` erhältst (mit den Werten "Jan", "Feb", "Mar") und eine Spalte `Verkauf` für die Verkaufszahlen.*
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
 *Wandle den Datensatz "rainfall_long" ins Wide-Format um, sodass jeder Monat eine eigene Spalte bildet, welche die Niederschlagswerte enthält.*
...
In diesem Abschnitt arbeiten wir mit einem Datensatz, der Informationen über Studierende sowie ihre Testergebnisse aus verschiedenen Semestern enthält.
...
Wir können uns den Datensatz anzeigen mit:
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Erstelle eine dplyr Pipe und überschreibe den Datensatz:*
...
<div class="tip">
  ...
  <div class="tip-content"> Weise den veränderten Datensatz wieder `students_data` zu.</div>
...
students_data
```
```{r ex-wrangling-1-1-check, ref.label=I('setup-ex-wrangling-df'), results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
<!-- necessary to transfer the changes from the previous task -->
```{r setup-ex-wrangling-df2, results = "hold", exercise.eval=FALSE}
students_data <- data.frame(
  stud_id = c("101","102","103","104","105"),
  name = c("Alex", "Ben", "Clara", "Dana", "Chris"),
  course = c("Biology", "Biology", "Psychology", "Physics", "Medicine"),
...
students_data <- students_data %>%
  ...
  filter(course != "Physics")
```
```{r, ref.label=I('setup-ex-wrangling-df2'), eval=TRUE, echo=FALSE, exercise.eval=FALSE}
```
Hier können wir uns den Datensatz anschauen, den wir bearbeiten möchten:


```{r data-ex-wrangling2, exercise.setup = "setup-ex-wrangling-df2", exercise=TRUE, results = "hold"}
students_data
```

`r otter::colorize("Aufgabe:", "var(--blue)")`
*Verwandle den Datensatz "students_data" so, dass eine Spalte "Semester" entsteht (mit Werten "Sem1", "Sem2", "Sem3") und eine Spalte "Test_Score", in der die Testergebnisse gespeichert sind.*
...
```{r ex-wrangling-2-1, exercise=TRUE, exercise.eval = TRUE, echo=TRUE, exercise.setup = "setup-ex-wrangling-df2", exercise.lines=10}
...
students_data
```
```{r ex-wrangling-2-1-check, ref.label=I('setup-ex-wrangling-df2'), results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
<a href="#section-aesthetics"> <i class="fa-solid fa-arrow-down"></i> aesthetics </a><br>
</div>
...
<a href="#section-weitere-linien"> <i class="fa-solid fa-arrow-down"></i> Weitere Linien </a><br>
...
**Schritte zur Erstellung eines Plots:**
  ...
  2. Die Spalten im Datensatz müssen den grafischen Elementen zugeordnet werden (x-Achse, y-Achse, Farben, Größen, ...).
...
### Ideen & Literatur <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>

Weiterführende Literatur: [Das ggplot2 Buch](https://ggplot2-book.org/)
...
### Beispieldatensatz <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
```
```{r, ref.label=I('setup-ggplot-df'), eval=TRUE, echo=TRUE, exercise.eval=FALSE}
```
</div></div></details>
...
### Piping mit `+` <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
Nachdem die `ggplot()` Funktion aufgerufen wurde, wird zu `+` gewechselt.
Danach wird jede Ebene des Plots mit `+` hinzugefügt.


### Data Mapping <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### Leere Plots erstellen <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
Es ist hier noch kein klassischer Inhalt zu sehen, sondern nur der leere Plot und die Achsen-Beschriftungen:
...
#### aesthetics <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
```{r data-mapping-05, exercise=FALSE, eval=FALSE, echo=TRUE, exercise.setup = "setup-ggplot-df", error=TRUE}
ggplot(
  df,
  aes(x = group,        # x-Achse
...
### Plot erstellen <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### Entscheidungshilfe für Plottypen <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_point() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_line() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### Weitere Linien <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
```{r geom-hline-02, exercise=TRUE, exercise.eval = TRUE, echo=TRUE, exercise.setup = "setup-ggplot-df"}
ggplot(df, aes(x = life_satisfaction, y = depression)) + 
  geom_point() + 
  geom_hline(yintercept = mean(df$depression), color = "blue")
```

```{r geom-vline-03, exercise=TRUE, exercise.eval = TRUE, echo=TRUE, exercise.setup = "setup-ggplot-df"}
ggplot(df, aes(x = life_satisfaction, y = depression)) + 
  geom_point() + 
  geom_vline(xintercept = mean(df$depression), color = "green")
```

#### geom_smooth() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_bar() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_col() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_histogram() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_boxplot() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_violin() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_density() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_area() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_ribbon() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_jitter() <a href="#section-ggplot2-plot-erstellen"> <i class="fa-solid fa-arrow-up"></i></a>
...
### Kapitel
<a href="#section-beschriftungen"> <i class="fa-solid fa-arrow-down"></i> Beschriftungen </a><br>
<div style="padding-left: 2em;">
<a href="#section-labs"> <i class="fa-solid fa-arrow-down"></i> labs() </a><br>
<a href="#section-geom_text"> <i class="fa-solid fa-arrow-down"></i> geom_text() </a><br>
<a href="#section-geom_label"> <i class="fa-solid fa-arrow-down"></i> geom_label() </a><br>
</div>
<a href="#section-themes-designs"> <i class="fa-solid fa-arrow-down"></i> Themes (Designs) </a><br>
<a href="#section-schriftgröße"> <i class="fa-solid fa-arrow-down"></i> Schriftgröße </a><br>
<a href="#section-farbskalen"> <i class="fa-solid fa-arrow-down"></i> Farbskalen </a><br>
<a href="#section-farben"> <i class="fa-solid fa-arrow-down"></i> Farben </a><br>
<a href="#section-plot-speichern"> <i class="fa-solid fa-arrow-down"></i> Plot speichern </a><br>

### Beschriftungen <a href="#section-ggplot2-plot-anpassen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### labs() <a href="#section-ggplot2-plot-anpassen"> <i class="fa-solid fa-arrow-up"></i></a>

Mit `labs()` kann man die üblichen Textbereiche beschriften: Titel, Achsen-Beschriftungen und weiteres.
...
#### geom_text() <a href="#section-ggplot2-plot-anpassen"> <i class="fa-solid fa-arrow-up"></i></a>
...
#### geom_label() <a href="#section-ggplot2-plot-anpassen"> <i class="fa-solid fa-arrow-up"></i></a>
...
### Themes (Designs) <a href="#section-ggplot2-plot-anpassen"> <i class="fa-solid fa-arrow-up"></i></a>
...
Wenn dir diese Themes nicht gefallen, nutze `theme()`, um Details zu ändern.
...
### Schriftgröße <a href="#section-ggplot2-plot-anpassen"> <i class="fa-solid fa-arrow-up"></i></a>
...
Ändere doch einfach Mal die Schriftgröße `base_size = 10` und führe den Code aus!
...
### Farbskalen <a href="#section-ggplot2-plot-anpassen"> <i class="fa-solid fa-arrow-up"></i></a>
...
### Farben <a href="#section-ggplot2-plot-anpassen"> <i class="fa-solid fa-arrow-up"></i></a>
...
### Plot speichern <a href="#section-ggplot2-plot-anpassen"> <i class="fa-solid fa-arrow-up"></i></a>


Mit `ggsave()` können wir ganz einfach unsere Plots speichern.
Die Funktion speichert immer den zuletzt erstellten Plot.
...
Hier findest du umfassende Übungen, welche die zentralen Inhalte der vorangegangenen Kapitel zum Thema Visualisierungen zusammenführen.
Die Aufgaben sind entsprechend anspruchsvoller gestaltet – selbstverständlich aber wie immer mit hilfreichen Tipps und Lösungen versehen.
...
`r otter::colorize("Aufgabe 1:", "var(--blue)")`
*Versuche den angezeigten Plot so ähnlich wie möglich nachzubauen.*

```{r ex-ggplot-0-0, exercise=FALSE, echo=TRUE, exercise.setup = "setup-ex-ggplot-df", eval=TRUE, echo=FALSE}
...
```{r ex-ggplot-0-1, exercise=TRUE, exercise.eval = TRUE, echo=TRUE, exercise.setup = "setup-ex-ggplot-df", exercise.lines=15}

```
```{r ex-ggplot-0-1-solution, ref.label=I('ex-ggplot-0-0')}

```

<br> 

`r otter::colorize("Aufgabe 2:", "var(--blue)")`
*Versuche den angezeigten Plot so ähnlich wie möglich nachzubauen.*
...
`r otter::colorize("Aufgabe 3:", "var(--blue)")`
*Versuche den angezeigten Plot so ähnlich wie möglich nachzubauen.*
...
<div class="tip">
  ...
  <div class="tip-content"> Nutze die Funktion `geom_point()`.</div>
  <div class="tip-content"> Nutze die Funktion `filter()`, um die Daten zu ändern.</div>
  ...
  <div class="tip-content"> Nutze hier `color` und nicht `fill` für die Variable des Geschlechts</div>
  <div class="tip-content"> Bearbeite die Beschriftungen mit `labs()`.</div>
  <div class="tip-content"> Nutze `scale_color_viridis_d()` für die Farben. Achte hierbei auf `color`.</div>

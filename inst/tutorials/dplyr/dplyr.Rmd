...
<a href="#section-spalte-zusammenfügen-mit-row_cols"> <i class="fa-solid fa-arrow-down"></i> Spalten zusammenfügen mit bin_cols </a><br>
</div><br>
...
Dieses Format ist häufig vorteilhaft für Datenanalysen und Visualisierungen (insbesondere auch `ggplot2`).
...
Jetzt schauen wir uns noch einen komplexeren Datensatz an in dem es mehr Spalten gibt.
...
Falls die Spaltennamen inkonsistent sind, sollte dies vor dem Pivoting idealerweise mit Hilfe von `dplyr` bereinigt werden.
...
**Erklärung:**
  ...
  - `names_sep = "_"`: Der Unterstrich ("_") dient als Trennzeichen.
...
**Grundstruktur und Elemente eines Plots:**

  - die verwendeten Daten

  - die Daten müssen auf die grafischen Elemente abgebildet werden (x-Achse, y-Achse, Farben, Größen, ...)

  - der Diagrammtyp wird festgelegt (Histogramm, Liniendiagramm, Balkendiagramm, Streudiagramm, ...)

  - das Diagramm wird individuell angepasst (Themen/Designs, zusätzliche Linien, Größen, Farben, Beschriftungen, ...)

  - `+` wird als eine Art Pipe-Operator verwendet, um weitere Ebenen zum Diagramm hinzuzufügen


### Ideen & Literatur

Buch: [ggplot2](https://ggplot2-book.org/)
...
Kunst: Gallerie von Danielle Navarro
...
df <- df %>% 
  mutate(
    ...
    sex = sample(c("female", "male", "unknown"), size = N, replace = TRUE, prob = c(0.50, 0.50, 0)),
...
Fall der Datensatz in einem anderen Format vorliegt, müssten wir zunächst den Datensatz mit pivoten.
Siehe dazu das Kapitel "tidyr - Pivoting".
...
Man kann die einzelnen Ebenen eines Plots auch pipen - es wird hier jedoch das `+` Symbol genutzt und nicht `%>%`.

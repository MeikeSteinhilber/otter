...
$(document).ready(function() {
  ...
  <a href="/"> <i class="fa-solid fa-bars" aria-label="Link zur otter Homepage" aria-hidden="false"></i>
  </a>
  </div>`);
  ...
  `<div class="logo">
  <img class="logo" src="images/corporate_logos_small.jpg" alt="Digtell Logo" style="width:70.0%"/>
   ...
   const cookiebanner = `<div class="modal modal-dialog-scrollable modal-dialog-centered modal-dialog-scrollable fade" id="staticBackdrop" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" style="justify-content: end; margin: unset !important;">
    <div class="modal-dialog" style="display: flex; align-self: end; margin: unset !important;">
        ...
        <div class="modal-footer">
            ...
            <button type="button" class="btn btn-light" id="cookie-notrack">Ich bin nicht einverstanden.</button>
        </div>
        </div>  
    </div>
    </div>`;
    ...
    if(consent == undefined || consent == "0") {
        ...
        jQuery('#cookie-notrack').on("click", function() {
            Cookies.set('cnsnt', '2', { expires: 30, sameSite: 'strict' });
            jQuery('#staticBackdrop').modal("hide");
        });
...
<br>
![RStudio Logo](images/RStudio-Logo-Flat.png){right="100%" width="20%"}
...
Nachdem RStudio geöffnet wurde, sieht es so aus:

![Die Console ist in einem Fenster geöffnet. Man kann dort die verwendete R Version “4.2.3” einsehen. Im rechten Drittel des Bildes ist das geöffnete Environment zu sehen. Direkt darunter ist ein weiteres Fenster, über das man zu Files, Plots, Packages, Help und Viewer gelangt.](images/RStudio_fresh_start.png){.absolute .images-shadow left="100%" width="80%"}
...
![Die Console ist geöffnet.](images/console.png){.absolute .images-shadow left="100%" width="80%"}
...
![Die Console ist geöffnet. Es werden beispielhafte Taschenrechner-Befehle ausgeführt, z. B. 5*6 = 30](images/console2.png){.absolute .images-shadow left="100%" width="60%"}


### Environment <a href="#section-rstudio-einstieg"> <i class="fa-solid fa-arrow-up"></i></a>

![Das Environment ist geöffnet. Im Environment steht Folgendes: Environment is empty. Über denselben Reiter kann man zu History, Connections und Tutorial gelangen.](images/environment.png){.absolute .images-shadow left="100%" width="80%"}
...
![Das Environment ist geöffnet und nicht mehr leer. Es werden gespeicherte Variablen und Datensätze angezeigt (z.B. data.frame und list).](images/environment2.png){.absolute .images-shadow left="100%" width="80%"}

### Files <a href="#section-rstudio-einstieg"> <i class="fa-solid fa-arrow-up"></i></a>

![Das Fenster „Files“ ist geöffnet. Dort befinden sich zwei Dokumente (.Rhistory und project.Rproj).](images/files.png){.absolute .images-shadow left="100%" width="80%"}
...
In einem R-Skript kann ein R Programmiercode abgespeichert werden.
...
![Ein R-Skript, in dem sich ein Code befindet, ist geöffnet.](images/script1.png){.absolute .images-shadow left="100%" width="80%"}
...
![Ein R-Skript, in dem sich ein Code befindet, ist geöffnet. Man sieht, dass ein Kommentar durch eine Raute am Zeilenanfang gekennzeichnet wird.](images/script1.png){.absolute .images-shadow left="100%" width="80%"}
...
![Ein R-Skript, in dem sich ein Code befindet, ist geöffnet. Der Code wurde blau markiert, dann wird auf den Run-Button geklickt, um den markierten Codeabschnitt auszuführen.](images/script2.png){.absolute .images-shadow left="100%" width="80%"}
...
![Ein neues, leeres R-Skript wird über die RStudio Oberfläche über ein grün-weißes Plus-Symbol geöffnet. Dieses befindet sich oben links.](images/button-rscript-open.png){.absolute .images-shadow left="100%" width="80%"}

Man erhält eine große Anzahl an möglichen Formaten, unter anderem auch ein R-Skript.

![Nachdem das Symbol mit dem grünen Kreis und dem weißen Plus gedrückt wurde, öffnen sich weitere Auswahlmöglichkeiten. Die oberste Leiste mit dem Titel „R Script - Create a new R script“ wird ausgewählt.](images/button-rscript-open2.png){.absolute .images-shadow left="100%" width="20%"}

Das neu geöffnete R-Skript sieht nun so aus und kann direkt mit dem Disketten-Symbol abgespeichert werden.

![Ein neues, leeres R-Skript ist geöffnet. Das zum Speichern geeignete blaue Disketten-Symbol befindet sich über der ersten Zeile.](images/empty-rscript-open.png){.absolute .images-shadow left="100%" width="80%"}
<div class="uebungen">
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
 *Klicke auf den Button "Code ausführen" und vergleiche die Ausgabe mit den obrigen Bildern*.
...
grade_result(
  ...
  glue_correct = feedback_message_correct,
  glue_incorrect = "Leider falsch!"
)
```
...
Variablen dienen als Speicherorte oder Container für Daten in R.
Sie ermöglichen es, Informationen wie Zahlen, Zeichenketten, Matrizen oder Listen effizient zu speichern und wiederzuverwenden.

Um eine Variable zu erstellen und zu verwenden, muss ihr Name mit einem Wert verknüpft werden, was durch Zuweisung geschieht.
Sobald dies erfolgt ist, erscheint die Variable im *Environment* von R, wo alle aktiven Objekte und deren Inhalte verwaltet werden.

Der Inhalt einer Variable kann in der Console angezeigt werden, indem der Variablenname eingegeben und ausgeführt wird.
Dies ermöglicht es, schnell auf den gespeicherten Wert zuzugreifen und ihn zu überprüfen.

```{r variable-0-1, exercise=TRUE, results = "hold"}
# hier wird der Text "Lara" gespeichert in der Variable "name"
name <- "Lara"
name

# hier wird eine Zahl gespeichert
lieblingszahl <- 3
lieblingszahl

# hier wird ein logischer Datentyp gespeichert
weiblich <- TRUE
weiblich

# es gibt auch Spezialfälle an Daten, die gespeichert werden können, wie das Nullobjekt
alter <- NULL
alter
```
...
![Das Environment ist geöffnet. Die gespeicherten Daten (alter = NULL, lieblingszahl = 3, name = “Meike”, weiblich = “TRUE” ) sind zu sehen. ](images/env_example.png){.absolute .images-shadow width="500"}

Mit Operatoren können Variablen Inhalte zugewiesen werden.
In R gibt es hierfür zwei Zuweisungsoperatoren: `<-` und `=`.
Wobei es als guter Stil gilt, einheitlich `<-` zu verwenden.


```{r variable-0-2, exercise=TRUE, results = "hold"}
...
```{r variable-0-3, exercise=TRUE, results = "hold"}
# Alte Daten
variable <- "Hallo!"
variable

# Der Inhalt wird nun überschrieben
variable <- 3
variable
```
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Erstelle ein Variable mit dem Inhalt: "Sonne". Und gebe Dir den Inhalt der Variable auf der Console aus.*
```{r ex-variable-0, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE}

```
```{r ex-variable-0-solution, results = "hold", exercise.eval=FALSE}
variable <- "Sonne"
variable
```
```{r ex-variable-0-check, results = "hold", exercise.eval=FALSE}
# grade_result(
...
<a href="#section-infektiöse-fehlende-werte"> <i class="fa-solid fa-arrow-down"></i> Infektiöse fehlende Werte</a><br>
...
Dieser Datentyp kann aufsummiert werden:

```{r logicals-1, exercise=TRUE, results = "hold"}
variable_true <- TRUE
variable_false <- FALSE

variable_true + variable_true 
variable_false + variable_false
variable_true + variable_false 
```


<details class="hiddendetails-container"> <summary>
<p class="hiddendetails-button"><i class="fa-solid fa-arrow-down"></i> Zeige eine zusätzliche Erklärung an <i class="fa-solid fa-arrow-down"></i> </p> </summary>
<div class="hiddendetails-body"> <div class="uebungen">

```{r logicals-1-1, exercise=FALSE, eval=FALSE, exercise.eval=FALSE, echo=TRUE}
variable_true <- TRUE
variable_false <- FALSE

# Hier wird also 1 + 1 gerechnet (TRUE + TRUE)
variable_true + variable_true 
# Hier wird also 0 + 0 gerechnet (FALSE + FALSE)
variable_false + variable_false
# Hier wird also 1 + 0 gerechnet (TRUE + FALSE)
variable_true + variable_false 
```
</div> </div> </details>
...
| `|`    | oder        |
| `&`    | und         |
| `!`    | Negierung (Umkehrung der Logik)   |

<!-- : Logische Operatoren -->

Mit logischen Operatoren können logische Prüfungen auf Objekte (Variablen, Vektoren, Datensätze, ...) angewandt werden.
...
# Welche Buchstaben sind ungleich C?
buchstaben != "C"
...
namen <- c("Peter Bauer", "Lisa Müller", "Hannah Schmidt")
alter <- c(18, 22, 16)
...
<div class="tip">
  ...
  <div class="tip-content"> namen ____ "Lisa Müller"</div>
  <div class="tip-content"> Nutze einen logischen Operator.</div>
  <div class="tip-content"> Nutze `!=`.</div>
...
`integer`-Zahlen sind Ganzzahlen in R.
...
<div class="tip">
  ...
  <div class="tip-content"> 5 ____ 12</div>
  <div class="tip-content"> Verwende den `:` Operator.</div>
...
grade_result(
  ...
  glue_correct = feedback_message_correct,
  glue_incorrect = "Leider noch nicht ganz richtig! Hinweis: Versuche den Operatur : zu nutzen."
)
```


`r otter::colorize("Aufgabe:", "var(--blue)")`
*Welcher Datentyp hat die Zahl `78`? Nutze zur Beantwortung der Frage eine Funktion.*
...
grade_this({ # .solution all, because the solutions have different outputs
  answer <- is.null(code_feedback(.user_code, .solution_code_all[["Alternativer Ansatz"]]))
  pass_if(
    answer,
    "`class()` gibt eine ähnliche Information aus, ist aber nicht identisch zu `typeof()`"
  )
  pass_if_equal(.solution_all)
  fail_if_not_equal(.solution_all, message = "Der Output stimmt nicht mit der Lösung überein. Eventuell hilft dir der Hinweis: {code_feedback()}")
})
```
...
Text wird häufig als Funktionsargument verwendet, weil hier auch alle Zeichen abgebildet werden können, z. B. um Symbole anzuzeigen.
...
In R gibt es viele Möglichkeiten, um auszudrücken, dass etwas nicht vorhanden ist.
...
0/0
# diese Rechenoperation ist mathematisch nicht definiert
```

`NA` tritt meistens auf, wenn ein Datensatz oder eine Variable einen fehlenden Wert enthält.
Es ist üblich eine solche fehlende Information mit `NA` zu speichern.
...
<!-- variable <- NULL # diese Variable wurde z.B. durch eine vorherige Operation zu NULL -->
<!-- # logische Vergleiche werden nun nachfolgend zu logical(0) -->
<!-- variable == "test" -->
<!-- variable == 5 -->
<!-- ``` -->
...
```{r na-6, exercise=TRUE, results = "hold"}
# Eine Variable wird unabsichtlich zu NaN
variable <- 0/0
...
```{r na-8, exercise=TRUE, results = "hold"}
variable <- NaN
...
```{r na-7, exercise=TRUE, results = "hold"}
variable <- NaN
variable | TRUE
```
...
<div class="tip">
  ...
  <div class="tip-carousel">
  <div class="tip-content"> Wie lautet die Funktion, welche auf fehlende Werte prüft?</div>
  <div class="tip-content"> Nutze `is.na()`</div>
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div>
...
```{r data-functions2-1, exercise=TRUE, results = "hold"}
# Erzeugung einer Variable
number <- 5

# Prüfung der Variable auf spezifische Datentypen
is.integer(number)
is.double(number)
is.numeric(number)
is.character(number)
...
```{r data-functions2-2, exercise=TRUE, results = "hold"}
# Erzeugung einer Variable
number <- 5
...
```{r data-functions3, exercise=TRUE, results = "hold"}
# Erzeugung einer Variable
number <- 0
...
```{r quiz-datentyp}
quiz(caption = "",
  question("In welchem Datentyp könnte ein solcher Smiley gespeichert werden: `:-)`? ",
    answer("numeric"),
    answer("logical"),
    answer("integer"),
  ...
  question("Wie werden Zahlen mit dem Datentyp `integer` erstellt?",
    answer("Mit einem L, z.B. `3L`", correct = TRUE),
    answer("`as.integer(3)`", correct = TRUE),
    answer("`1:3`", correct = TRUE),
    answer("Mit einem I, z.B. `3I`"),
    answer("Mit dem Eintippen der Zahl `3`"),
     random_answer_order = TRUE,
     allow_retry = TRUE,
     try_again = "Leider falsch!",
     correct = "Richtig!",
  ...
  question("Welche Aussagen sind richtig?",
    answer("`is.double()` prüft, ob es sich um den Datenyp double handelt", correct = TRUE),
    answer("`as.integer(3)` erstellt 3 Zahlen"),
    answer("`double(3)` erstellt die Zahl 3 im Datentyp double"),
    answer("Man kann 5 Nullen mit `integer(5)` erstellen", correct = TRUE),
     random_answer_order = TRUE,
     allow_retry = TRUE,
     try_again = "Leider falsch!",
     correct = "Richtig!",
...
Eine Variable kann verschiedene Inhalte mit verschiedenen Datentypen und Datenstrukturen enthalten, z. B. eine Zahl, Buchstaben, Matrizen mit Zahlen oder Listen.
...
```{r variable-3-1, exercise=TRUE, results = "hold"}
# Alte Daten
freunde <- 1:5
freunde
...
```{r variable-3-2, exercise=TRUE, results = "hold"}
# Alte Daten
freunde <- c("Lisa", "Thorben", "Mara")
freunde
...
```{r variable-4-1, exercise=TRUE, results = "hold"}
...
```{r variable-4-2, exercise=TRUE, results = "hold"}
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Erstelle ein Variable mit dem Inhalt: Baum, Blume, Haus, Pferd. Und gebe Dir den Inhalt der Variable auf der Console aus.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Nutze `c()`.</div>
  
  <!-- <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button> -->
  <!-- <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button> -->
</div> </details> </div>
```{r ex-variable-1, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE}

```
```{r ex-variable-1-solution, results = "hold", exercise.eval=FALSE}
variable <- c("Baum", "Blume", "Haus", "Pferd")
variable
```
```{r ex-variable-1-check, results = "hold", exercise.eval=FALSE}
grade_this_code()
```
</div>
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Ändere den Inhalt der Variable: entferne die Einträge `Hund`.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Denke daran das neue Objekt wieder `blumen` zuzuweisen.</div>
  <div class="tip-content"> Nutze `blumen <- blumen[____]`.</div>
  <div class="tip-content"> Nutze den logischen Operator `!=`.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-vector-1, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
blumen <- c("Tulpen", "Rosen", "Hund", "Sonnenblumen", "Hund")
# die erste Zeile soll unverändert bleiben!
# schreibe hier deinen Code
```
```{r ex-vector-1-solution, results = "hold", exercise.eval=FALSE}
# option 1 ----
blumen <- c("Tulpen", "Rosen", "Hund", "Sonnenblumen", "Hund")
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Ändere den Inhalt der Variable: ersetze die Einträge `Hund` durch `NA`.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  ...
  <div class="tip-content"> Denke daran das neue Objekt wieder `blumen` zuzuweisen.</div>
  <div class="tip-content"> Nutze den logischen Operator `==`.</div>
  <div class="tip-content"> `NA` kann zugeordnet werden mit `variable <- NA`.</div>
  <div class="tip-content"> Nutze `blumen[____] <- NA`.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-vector-2, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
blumen <- c("Tulpen", "Rosen", "Hund", "Sonnenblumen", "Hund")
```
```{r ex-vector-2-solution, results = "hold", exercise.eval=FALSE}
blumen <- c("Tulpen", "Rosen", "Hund", "Sonnenblumen", "Hund")
blumen[blumen == "Hund"] <- NA
blumen
```
```{r ex-vector-2-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Addiere auf jedes Element im Vektor die Zahl 3.*
```{r ex-vectorisiert-1, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 8}
zahlen <- 1:100
```
```{r ex-vectorisiert-1-solution, results = "hold", exercise.eval=FALSE}
zahlen <- 1:100
zahlen + 3
```
```{r ex-vectorisiert-1-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Multipliziere jedes Element im Vektor mit der Zahl 5.*
```{r ex-vectorisiert-2, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 8}
zahlen <- 1:100

```
```{r ex-vectorisiert-2-solution, results = "hold", exercise.eval=FALSE}
zahlen <- 1:100
zahlen * 5
```
```{r ex-vectorisiert-2-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Multipliziere die zwei Vektoren miteinander.*
```{r ex-vectorisiert-3, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 8}
nummer_1 <- 1:100
nummer_2 <- 100:1
```
```{r ex-vectorisiert-3-solution, results = "hold", exercise.eval=FALSE}
nummer_1 <- 1:100
nummer_2 <- 100:1

nummer_1 * nummer_2
```
```{r ex-vectorisiert-3-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Erstelle einen Faktor aus der Variable `var_monate`. Sortiere danach die Monate innerhalb dieser Variable.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Nutze `factor()`.</div>
  <div class="tip-content"> Nutze `sort()`.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-factor-1, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 8}
...
```
```{r ex-factor-1-solution, results = "hold", exercise.eval=FALSE}
monate <- c("Januar", "Februar", "März", "April", "Mai", "Juni",
            "Juli", "August", "September", "Oktober", "November", "Dezember")
var_monate <- c("Juli", "Mai", "Januar", "November")
...
sort(var_monate)
```
```{r ex-factor-1-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Erstelle eine Matrix aus der Variable `farben`. Wähle eine inhaltlich sinnvolle Aufteilung für die Spalten.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Nutze `matrix()`.</div>
  <div class="tip-content"> Nutze das Funktionsargument `ncol` innerhalb von `matrix()`.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-matrix-1, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
farben <- c("rot", "hellrot", "blau", "dunkelblau", "grün", "hellgrün")
```
```{r ex-matrix-1-solution, results = "hold", exercise.eval=FALSE}
# vereinfachte Lösung ----
matrix(c("rot", "hellrot", "blau", "dunkelblau", "grün", "hellgrün"), ncol = 3)
...
farben
```
```{r ex-matrix-1-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({ # .solution, because the two solutions have the same output
  pass_if_equal(.solution, message = feedback_message_correct)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Wähle die zweite Spalte aus der Matrix aus.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Nutze `[]` um auf den Inhalt zuzugreifen.</div>
  <div class="tip-content"> Vergesse nicht das Komma und achte auf die Reihenfolge!</div>
  <div class="tip-content"> Die Reihenfolge: `[Zeile,Spalte]`.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-matrix-2, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
daten <- matrix(c("Lisa", "Thorben", "Mara", "TRUE", "FALSE", "TRUE", "2", "5", "8")
                , ncol = 3)
```
```{r ex-matrix-2-solution, results = "hold", exercise.eval=FALSE}
daten <- matrix(c("Lisa", "Thorben", "Mara", "TRUE", "FALSE", "TRUE", "2", "5", "8")
                , ncol = 3)
daten[, 2]
```
```{r ex-matrix-2-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Wähle aus der ersten Spalte die dritte Zeile aus.*
```{r ex-matrix-3, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
daten <- matrix(c("Lisa", "Thorben", "Mara", "TRUE", "FALSE", "TRUE", "2", "5", "8")
                , ncol = 3)
```
```{r ex-matrix-3-solution, results = "hold", exercise.eval=FALSE}
daten <- matrix(c("Lisa", "Thorben", "Mara", "TRUE", "FALSE", "TRUE", "2", "5", "8")
                , ncol = 3)
daten[3, 1]
```
```{r ex-matrix-3-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
![Es wird gezeigt, dass mit $ auf Spalten zugegriffen werden kann.](images/dollar_operator.png){width="300"}
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Erstelle einen Datensatz `tiere` aus den Variablen und gebe den Datensatz aus.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Nutze `data.frame()`.</div>
  
  <!-- <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button> -->
  <!-- <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button> -->
</div> </details> </div>
```{r ex-df-1, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 11}
tierart <- c("Hund", "Katze", "Maus", "Schlange", "Hund", "Hund", "Katze")
alter <- c(12, 2, 1, 4, 6, 2, 3)
gesund < c(TRUE, FALSE, TRUE, TRUE, FALSE, FALSE, TRUE)

```
```{r ex-df-1-solution, results = "hold", exercise.eval=FALSE}
tierart <- c("Hund", "Katze", "Maus", "Schlange", "Hund", "Hund", "Katze")
alter <- c(12, 2, 1, 4, 6, 2, 3)
...
tiere
```
```{r ex-df-1-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Wähle die Spalte `zahl` aus (verwende dazu den Namen der Spalte).*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Nutze den `$` Operator.</div>
  
  <!-- <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button> -->
  <!-- <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button> -->
</div> </details> </div>
```{r ex-df-2, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 10}
data <- data.frame(
...
```
```{r ex-df-2-solution, results = "hold", exercise.eval=FALSE}
data <- data.frame(
  buchstabe = c("A", "B", "C", "D", "E", "F", "G"),
  zahl = c(12, 2, 1, 4, 6, 2, 3),
  verwendet = c(TRUE, FALSE, TRUE, TRUE, FALSE, FALSE, TRUE)
...
data$zahl
```
```{r ex-df-2-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Wähle die Spalte `buchstabe` aus und ersetze den Buchstaben `D` mit einem `X`.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> `data____ <- ____`</div>
  <div class="tip-content"> `data$____ <- ____`</div>
  <div class="tip-content"> `data$____[____] <- ____`</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-df-3, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 10}
data <- data.frame(
  buchstabe = c("A", "B", "C", "D", "E", "F", "G"),
  zahl = c(12, 2, 1, 4, 6, 2, 3),
  verwendet = c(TRUE, FALSE, TRUE, TRUE, FALSE, FALSE, TRUE)
...
```
```{r ex-df-3-solution, results = "hold", exercise.eval=FALSE}
data <- data.frame(
  buchstabe = c("A", "B", "C", "D", "E", "F", "G"),
  zahl = c(12, 2, 1, 4, 6, 2, 3),
  verwendet = c(TRUE, FALSE, TRUE, TRUE, FALSE, FALSE, TRUE)
...
data$buchstabe[4] <- "X"
```
```{r ex-df-3-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
Je nachdem, um was für ein Objekt es sich in der Liste handelt und wie darauf zugegriffen werden soll, müssen somit einfache oder doppelte eckige Klammern verwendet werden.
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Erstelle eine Liste aus den drei Objekten.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Nutze `list()`.</div>
  
  <!-- <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button> -->
  <!-- <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button> -->
</div> </details> </div>
```{r ex-df-00, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 11}
m <- matrix(1:12, nrow=3, ncol=4)
df <- data.frame(namen = c("Ben", "Claus", "Lisa"),
                 alter = c(51, 27, 38))
object <- "Object 3"
```
```{r ex-df-00-solution, results = "hold", exercise.eval=FALSE}
m <- matrix(1:12, nrow=3, ncol=4)
df <- data.frame(namen = c("Ben", "Claus", "Lisa"),
                 alter = c(51, 27, 38))
liste <- list(m,
...
liste
```
```{r ex-df-00-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Greife auf den Inhalt des zweiten Objektes der Liste zu.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Verwende eckige Klammern.</div>
  
  <!-- <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button> -->
  <!-- <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button> -->
</div> </details> </div>
```{r ex-df-0-1, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 7}
liste <- list("test",
              matrix(1:12, nrow=3, ncol=4),
              data.frame(namen = c("A", "B", "C"),
                 alter = c(20, 00, 50)))
```
```{r ex-df-0-1-solution, results = "hold", exercise.eval=FALSE}
liste <- list("test",
              matrix(1:12, nrow=3, ncol=4),
              data.frame(namen = c("A", "B", "C"),
                 alter = c(20, 00, 50)))
liste[[2]]
```
```{r ex-df-0-1-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
...
<a href="#section-hilfreiche-funktionen-1"> <i class="fa-solid fa-arrow-down"></i> Hilfreiche Funktionen</a><br>

### Funktionen erstellen <a href="#section-funktionen"> <i class="fa-solid fa-arrow-up"></i></a>

Funktionen sind elegante Möglichkeiten, um Code-Abschnitte zu bündeln, zu strukturieren und wiederzuverwenden.
Eine Funktion tut irgendetwas mit Daten, Dokumenten, Argumenten etc.
...
Optionale Argumente können auch mit argument = NULL angegeben werden.
Das ist eher ein Spezialfall, aber man sieht es manchmal in der Dokumentation auf den Hilfe-Seiten von Funktionen.
Wenn ihr das Argument nicht benötigt, müsst ihr es auch nicht definieren und es sind auch keine klassischen Default-Werte hinterlegt.

```{r funktionen-5, exercise=TRUE, results = "hold"}
add_numbers <- function(number1 = 1, number2 = 1, custom_text = NULL) {
  result <- number1 + number2
  if (!is.null(text)) { # Ist ein Wert vorhanden in der Variable?
    result <- paste(custom_text, result)
  }
  return(result)
}
add_numbers(5,80)
add_numbers(5,80, custom_text = "result =")
add_numbers(5,80, custom_text = "The answer is")
```
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Schreibe eine Funktion, die zwei Zahlen addiert. Nutze dazu den bereits angegebenen Code.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Der Basiscode der Funktion ist bereits angegeben. Es fehlen noch die Funktionsargumente und der Körper der Funktion, also was mit den Funktionsargumenten geschehen soll.</div>
  <div class="tip-content"> Überlege wie viele Funktionsargumente benötigt werden.</div>
  <div class="tip-content"> Nutze zwei Funktionsargumente (pro Zahl ein Argument) - die Benennung kann selbst gewählt werden.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-functions-00, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
rechner <- function() {
...
```
```{r ex-functions-00-solution, results = "hold", exercise.eval=FALSE}
rechner <- function(zahl1, zahl2) {
  zahl1 + zahl2
}
rechner(8, 5)
```
```{r ex-functions-00-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Schreibe eine Funktion, die zwei Vektoren multipliziert.*
```{r ex-functions-01, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 8}
# Schreibe hier die Funktion

# Teste die Funktion
vektor_rechner(c(1,2,3), c(3,2,1)) # Diese Zeile soll nicht verändert werden
```
```{r ex-functions-01-solution, results = "hold", exercise.eval=FALSE}
vektor_rechner <- function(vektor1, vektor2) {
  vektor1 * vektor2
}
vektor_rechner(c(1,2,3), c(3,2,1))
```
```{r ex-functions-01-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Schreibe eine Funktion, die den Mittelwert über einen Vektor berechnet und folgende Ausgabe macht: `"Mittwelwert = 5.67"`.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  ...
  <div class="tip-content"> Der Basiscode der Funktion ist bereits angegeben. Es fehlen noch die Funktionsargumente und der Körper der Funktion, also was mit den Funktionsargumenten geschehen soll.</div>
  <div class="tip-content"> Verwende ein Funktionsargument für den Zahlenvektor.</div>
  <div class="tip-content"> Baue die Funktion Stück für Stück zusammen. Teste auch bereits Zwischenschritte.</div>
  <div class="tip-content"> Verwende die Funktionen: `round()` und `mean()` und wende es auf den Zahlenvektor (das Funktionsargument) an.</div>
  <div class="tip-content"> `____ <- round(mean(___), 2)`</div>
  <div class="tip-content"> Verwende die Funktionen: `paste()`, um den Satz auszugeben.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-functions-02, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 8}
# Schreibe hier die Funktion

# Teste die Funktion
vektor_mittelwert(c(1,2,3, 8, 8, 12)) # Diese Zeile soll nicht verändert werden
```
```{r ex-functions-02-solution, results = "hold", exercise.eval=FALSE}
# Lösung:
...
```
```{r ex-functions-02-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Schreibe die vorhandene Funktion so um, dass der Code `taschen_rechner()` die folgende Rechnung ausführt: `1 + 2`.*
...
<div class="tip">
  ...
  <div class="tip-content"> Warum gibt die Funktion ein Ergebnis aus, wenn `taschen_rechner()` ausgeführt wird?.</div>
  <div class="tip-content"> Definiere Default-Argumente für den Fall `taschen_rechner()`.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-functions-03, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 8}
...
```
```{r ex-functions-03-solution, results = "hold", exercise.eval=FALSE}
taschen_rechner <- function(zahl1 = 1, zahl2 = 2, zeichen = "+") {
  text = paste(zahl1, zeichen, zahl2)
  eval(parse(text = text))
}
# Teste die Funktion
taschen_rechner(4, 5, "+")
taschen_rechner() # Diese Zeile soll nicht verändert werden
```
```{r ex-functions-03-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
Die Hilfe sollte immer verwendet werden, bevor man eine neue Funktion verwendet, um sich die Argumente und die Funktionsweise anzuschauen.
...
![Der Help-Reiter wurde ausgewählt in RStudio. Die Inhalte der mean() Funktion werden wiedergegeben.](images/help_mean_long.png){height="600"}
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Öffne hier die Hilfeseiten der Funktionen, um die folgenden Quiz-Fragen zu beantworten. Probiere auch gerne Code aus, um zu verstehen, wie die Funktionen funktionieren.*
```{r ex-help-00, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
...
```{r quiz-help}
quiz(caption = "",
  question("Was macht die `floor()` Funktion?",
    answer("Sie rundet eine Zahl auf."),
    answer("Sie berechnet einen mathematischen Grenzwert."),
    answer("Sie rundet Zahlen ab.", correct = TRUE),
     random_answer_order = TRUE,
     allow_retry = TRUE,
     try_again = "Leider falsch!",
     correct = "Richtig!",
  ...
  question("Welche Aussagen stimmen zu über die `all()` Funktion?",
    answer("Die Funktion prüft, ob alle Argumente in einer Funktion vorhanden sind."),
    answer("Die Funktion prüft, ob alle Werte in einem Vektor `TRUE` sind.", correct = TRUE),
    answer("Die Funktion hat zwei Pflicht-Argumente"),
    answer("Die Funktion hat ein Pflicht-Argument und ein Default-Argument.", correct = TRUE),
     random_answer_order = TRUE,
     allow_retry = TRUE,
     try_again = "Leider falsch!",
     correct = "Richtig!",
   ...
   question("Welche Aussagen stimmen zu über das `na.rm` Argument (Bsp. `all()`)?",
    answer("Das `na.rm` Argument entfernt alle `NA` Werte in einem Vektor vor der Berechnung.", correct = TRUE),
    answer("Das Argument ist in allen mathematischen Funktionen enthalten."),
    answer("Das Argument ist meist auf `TRUE` gesetzt."),
    answer("Das `na.rm` Argument ist in der log() Funktion enthalten."),
     random_answer_order = TRUE,
     allow_retry = TRUE,
     try_again = "Leider falsch!",
     correct = "Richtig!",
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Die Argumente werden im folgenden Code falsch übergeben. Korrigiere den Fehler.*
```{r ex-functions-05, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
median(TRUE, c(1:8))
```
```{r ex-functions-05-solution, results = "hold", exercise.eval=FALSE}
median(c(1:8), TRUE)
```
```{r ex-functions-05-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Die Argumente werden im folgenden Code falsch übergeben. Korrigiere den Fehler.*
```{r ex-functions-06, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
sample(1:10, NULL, TRUE, 4)
```
```{r ex-functions-06-solution, results = "hold", exercise.eval=FALSE}
# Lösung 1 ----
...
```
```{r ex-functions-06-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Löse die Problemstellung mit einer Funktion.*
```{r ex-functions-08, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
vektor <- c(rep(c(1:5), 4), 12, 19)
# Gebe alle Zahlen aus, die nur einmal vorkommen im Vektor

```
```{r ex-functions-08-solution, results = "hold", exercise.eval=FALSE}
vektor <- c(rep(c(1:5), 4), 12, 19)
# Gebe alle Zahlen aus, die nur einmal vorkommen im Vektor
unique(vektor)
```
```{r ex-functions-08-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Löse die Problemstellung mit einer Funktion.*
```{r ex-functions-09, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
vektor <- 1:10
# Gebe jedes Element im Vektor 5 mal aus,
# wobei zuerst 5 1-er und dann fünf 2-er, etc ausgegeben werden.
```
```{r ex-functions-09-solution, results = "hold", exercise.eval=FALSE}
vektor <- 1:10
# Gebe jedes Element im Vektor 5 mal aus,
# wobei zuerst 5 1-er und dann fünf 2-er, etc ausgegeben werden.
rep(vektor, each = 5)
```
```{r ex-functions-09-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
Dadurch werden alle Funktionen in den aktuellen Namensraum geladen und Funktionen können ohne den Präfix genutzt werden.
...
Die Übungen zu diesem Kapitel befinden sich hier:
 ...
 - <a href="#section-datenstrukturen"> <i class="fa-solid fa-arrow-down"></i> Datenstrukturen: data.frame</a><br>
...
```{r data-2, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
head(df, n = 10) # zeigt die ersten 10 Zeilen im Datensatz
```

```{r data-3, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
...
```{r data-4, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
summary(df)
```

```{r data-5, exercise=TRUE,results='hold', exercise.setup = "setup-data"}
psych::describe(df)
```

### Fehlende Werte anzeigen <a href="#section-datenverarbeitung"> <i class="fa-solid fa-arrow-up"></i></a>

```{r data-6, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
rowSums(is.na(df)) # Anzahl von fehlenden Werten pro Zeile (eine Zeile ist eine Person)
```

```{r data-7, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
...
```{r data-8, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
complete.cases(df) # Prüft, ob alle Spalten ausgefüllt wurden pro Zeile
```

```{r data-9, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
df[!complete.cases(df), ] # Zeigt die Daten, die fehlende Werte enthalten
```

```{r data-10, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
df[complete.cases(df), ] # Zeigt die Daten, die keine fehlenden Werte enthalten
```

Verwendung von `na.omit()`

```{r data-11, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
...
```{r data-13, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
# Im Datensatz df alle Personen die jünger sind als 18 Jahre
...
```{r data-14, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
...
```{r data-15, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
...
```{r data-16, exercise=TRUE, results='hold', exercise.setup = "setup-data"}
...
R Projekte erlauben es, alle Pfade nur in Bezug auf das Projekt zu definieren.
...
Pfade werden nun immer innerhalb eines Projektes referenziert.
Egal wo sich das Projekt befindet, die Pfade sind immer gleich.
...
![Es wird rechts oben in RStudio auf „Project: (None)“ und anschließend in der obersten Leiste auf „New Project“ geklickt.](images/project_1.png){ width="400"} <br>

Hier wählt man aus, ob man einen neuen Ordner anlegen möchte, oder ein bereits existierendes Verzeichnis nutzen möchte.
<br>
![Zu sehen sind die drei Auswahlmöglichkeit: New Directory, Existing Directory, and Version Control.](images/project_2.png){ width="450"} <br>

Hier können Vorlagen für Projekte ausgewählt werden.
In den meisten Fällen werdet ihr diese jedoch nicht benötigen.

<br>
![Zu sehen sind Vorlagen für Projekte. Sofern man diese nicht benutzt, muss man einfach in der obersten Leiste auf „New Project“ klicken.](images/project_3.png){ width="450"} <br>

Hier könnt ihr nun den Namen vergeben und den Pfad auswählen.<br>
![Zu sehen ist ein Eingabefeld, in dem man dem zu erstellenden Projekt einen Namen vergeben und den Projektpfad anpassen kann.](images/project_4.png){ width="450"} <br>

Nachdem ein Projekt erstellt wurde, befindet sich die `projektname.Rproj` Datei in eurem Projektordner.
Von nun an, muss diese Datei zuerst geöffnet werden, wenn man an dem Projekt arbeiten möchte.

<br>
![Zu sehen ist das erstellte Projekt in dem Projektordner. Es ist nur die Datei test.Rproj zu sehen.](images/project_5.png){ width="450"} <br>
...
![Zu sehen ist das erstellte Projekt in dem Projektordner. Es ist nur die Datei test.Rproj zu sehen.](images/project_5.png){ width="300"}
...
![Auf dem Screenshot sind oben ein Ordner namens „data“, darunter ein Ordner namens „images“, darunter ein Ordner namens „R“ und darunter die neue Projekt-Datei (test.Rproj) abgebildet.](images/project_6.png){ width="300"}
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Schreibe ein if-else-Statement, dass den ersten Satz ausgibt, wenn die Variable `tier` ein `Reptil` enthält. Der zweite Satz soll ausgegeben werden, wenn es sich nicht um ein `Reptil` handelt.*
```{r ex-ifelse-01, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 8}
tier <- c("Reptil")
# Schreibe hier deinen Code
...
```
```{r ex-ifelse-01-solution, results = "hold", exercise.eval=FALSE}
# Lösung 1 ----
tier <- c("Reptil")
if (tier == "Reptil") {
...
```
```{r ex-ifelse-01-check, results = "hold", exercise.eval=FALSE}
# grade_result(
#   pass_if(~identical(.result, 
...
<details class="hiddendetails-container"> <summary>
    <p class="hiddendetails-button"><i class="fa-solid fa-arrow-down"></i> Zeige die Antwort <i class="fa-solid fa-arrow-down"></i> </p>
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Schreibe eine For-Loop, die den Satz `Ich mag ____.` für alle Tiere ausgibt. Nutze dazu den Code, der bereits angegeben ist.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Der Output soll so aussehen: <pre>
    [1] "Ich mag Hunde." 
    [1] "Ich mag Katzen."
    [1] "Ich mag Vögel."</pre></div>
  <div class="tip-content"> Die Ausgabe kann mit paste gemacht werden: `print(paste0("Ich mag ", ____, "."))`</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-for-01, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 8}
tiere <- c("Hunde", "Katzen", "Vögel") # diese Zeile nicht ändern


```
```{r ex-for-01-solution, results = "hold", exercise.eval=FALSE}
# Lösung 1 ----
...
```
```{r ex-for-01-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
![Die Console ist geöffnet. “Hallo Welt” wird unendlich oft in der Console ausgegeben. Oben rechts befindet sich ein rotes Symbol, auf dem „Stop“ steht. Dieses muss geklickt werden, um die Ausführung zu stoppen.](images/while_endless.png){.absolute width="500"}
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Schreibe diese For-Loop in eine While-Loop um. Nutze dazu den Code, der bereits angegeben ist.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Achte darauf, dass die while-Loop eine break-Bedingung benötigt!</div>
  <div class="tip-content"> Verwende eine Zählvariable z.B `i = 1`.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-while-01, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 10}
tiere <- c("Hunde", "Katzen", "Vögel")
results <- character(3)
for (i in 1:length(tiere)) {
...
results
```
```{r ex-while-01-solution, results = "hold", exercise.eval=FALSE}
tiere <- c("Hunde", "Katzen", "Vögel")
results <- character(3)
i = 1
...
results
```
```{r ex-while-01-check, results = "hold", exercise.eval=FALSE}
# grade_this_code()
grade_result(
  pass_if(~identical(.result,
                     results
...
`r otter::colorize("Aufgabe:", "var(--blue)")`
*Schreibe eine Warnung, die angezeigt wird, wenn die Variable `zahl` nicht numerisch ist. Nutze dazu den Code der bereits vorhanden ist.*
<div class="tip">
  <details>
  <summary><img src="images/small_otter.png" alt="Otter Icon" class="icon-small">Tipp</summary>
  <div class="tip-carousel">
  
  <div class="tip-content"> Nutze `warning()`.</div>
  <div class="tip-content"> Verwende auch eine if-Abfrage.</div>
  
  <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button>
  <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button>
</div> </details> </div>
```{r ex-warning-01, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 5}
zahl <- "5"

"Zahl ist nicht numerisch!"
```
```{r ex-warning-01-solution, results = "hold", exercise.eval=FALSE}
zahl <- "5"
if (!is.numeric(zahl)){
  warning("Zahl ist nicht numerisch!")
}
```
```{r ex-warning-01-check, results = "hold", exercise.eval=FALSE}
grade_this({
  pass_if_equal(.solution, message = feedback_message_correct)
  fail_if_not_equal(.solution, message = feedback_message_wrong)
})
...
<a href="#section-in-funktionen"> <i class="fa-solid fa-arrow-down"></i> Funktionen</a><br>
<a href="#section-in-loops"> <i class="fa-solid fa-arrow-down"></i> Loops</a><br>

Variablen und Funktionen sind `Environments` (Umgebungen) zugeordnet.
Diese Zuordnung entscheidet, wo eine Variable oder eine Funktion existiert

### In Funktionen <a href="#section-environments"> <i class="fa-solid fa-arrow-up"></i></a>
`r otter::colorize("Vorbereitung:", "var(--red)")`
Wir erzeugen hier eine Globale-Variable `global_variable` und eine Variable, die innerhalb des Körpers einer Funktion erzeugt wird `function_variable`.
...
### In Loops <a href="#section-environments"> <i class="fa-solid fa-arrow-up"></i></a>
...
<div class="tip">
  ...
  <div class="tip-content"> Prüfe die logischen Operatoren.</div>
  <div class="tip-content"> Prüfe die Klammern.</div>
  <div class="tip-content"> Prüfe die Formatierung der Absätze.</div>
  <div class="tip-content"> Nutze `\n` für die Absätze.</div>
...
<div class="tip">
  ...
  <div class="tip-content"> Definiere die Inhalte direkt bei der Erstellung des Datensatzes.</div>

  <!-- <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button> -->
  <!-- <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button> -->
</div> </details> </div>
```{r ex-refactor-0, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 15}
...
<div class="tip">
  ...
  <div class="tip-content"> Mache aus der letzten if-else-Bedingung eine else-Bedingung.</div>

  <!-- <button class="prev-tip"><i class="fa-solid fa-arrow-left"></i></button> -->
  <!-- <button class="next-tip"><i class="fa-solid fa-arrow-right"></i></button> -->
</div> </details> </div>
```{r ex-refactor-1, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 15}
content <- 55 
if (content < 55) {
  print("Option 1")
} else if (content > 55) {
...
```{r ex-refactor-2, exercise=TRUE, results = "hold", exercise.eval=FALSE, error=TRUE, exercise.lines = 15}
namen <- c("Lisa", "Peter", "Klara", "Thorben")
...
<details class="hiddendetails-container"> <summary>
  ...
  </summary> <div class="hiddendetails-body">
...
### Programmierübungen <a href="#section-übergeordnete-übungen"> <i class="fa-solid fa-arrow-up"></i></a>
...
```{r ex-difficult-01, exercise=TRUE, exercise.eval=FALSE, results='hold', exercise.lines = 35}
...
```{r ex-free-01, exercise=TRUE, exercise.eval=FALSE, results='hold', exercise.lines = 8}

```

Arbeitsbereich 2 (mittel)

```{r ex-free-02, exercise=TRUE, exercise.eval=FALSE, results='hold', exercise.lines = 30}

```

Arbeitsbereich 3 (lang)
```{r ex-free-03, exercise=TRUE, exercise.eval=FALSE, results='hold', exercise.lines = 60}
